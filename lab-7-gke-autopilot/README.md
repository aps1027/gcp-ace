# GKE Autopilot — Infrastructure Overview

This lab deploys a production-style backend stack on a GKE Autopilot cluster, provisioned with Terraform and delivered via Cloud Build CI/CD.
      ...
      subgraph "VPC Network"
        ...
        subgraph "GKE Autopilot Cluster"
          ...
          subgraph "Kubernetes Resources (k8s-dev)"
            ...
            config["ConfigMap"]
            secret["Secret"]
            api_hpa["HPA: api (optional)"]
            auth_hpa["HPA: auth (optional)"]
          end
...
- `modules/gke/` — provisions a GKE Autopilot cluster (regional, Google-managed compute) and a global static IP for the Ingress. No node pool or node SA — Autopilot manages all compute automatically.
...
- `databases/mysql/statefulset.yaml` — deploys MySQL as a `StatefulSet`. Uses `storageClassName: standard-rwo` (GCE Persistent Disk, pd-balanced) with `10Gi` storage — GCE PD minimum. Resource requests are mandatory in Autopilot.
...
- `cloudbuild-api-gateway.yaml` — builds, pushes, and deploys the `api` image on every push. Uses `CLOUDSDK_COMPUTE_REGION` (not zone) because Autopilot clusters are regional.
...
Creates: VPC, GKE Autopilot cluster, Artifact Registry, Cloud Build triggers, static IP, and all IAM bindings.
...
- The static IP name (`gke-autopilot-ingress-ip`) is reserved by Terraform and referenced in the Ingress annotation. Run `terraform output ingress_ip_address` to get the address.
- **Autopilot requires resource `requests` and `limits` on every container** — these are used for billing and pod scheduling. All workloads in this lab include them.
- There is no node pool to manage. Autopilot provisions and scales compute automatically based on pod resource requests.
